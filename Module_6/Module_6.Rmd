# Analyse de variance à un critère {#lecon6}
...
##### Vous utiliserez les fonctions suivantes : {-} 
  ...
  - `qqnorm( )`, `qqline( )`
  - `boxplot( )`
  - `plot( )`, `axis( )`, `points( )`, `arrows( )`, `text( )`, `par( )`
  - `ptukey( )`
  - `TukeyHSD( )`
  - `rep( )`
  - `for( )`
...
Dans la partie Lecon, les fichiers suivantes sont utilisés :
  ...
  - Le fichier `jardins.txt` sur la concentration d’ozone en parties par million (ppm) diffère entre trois jardins d’une ville industrielle. 
  - Le fichier `consommation.txt` sur la consommation de boissons sucrées par les jeunes de 19 à 25 ans dans cinq régions de l’Est du Québec.

Dans la partie Autoévaluation, le fichier `Survie.txt` est utilise. Il présente le temps de survie en heures d’animaux exposés à trois doses de poison.   

Les données peuvent être téléchargées [ici](https://github.com/sci1018/sci1018_data/raw/main/Module_6.zip).
  </div>
...
df <- data.frame(
  ...
  Col4 = c("$SSA/df_A$", "$SSE/df_{erreur}$", " "),
  Col5 = c("$MSA/MSE$", " ", " ")
  )

colnames(df) <- c("Source", "Somme des carrés ($SS$)", "Degrés de liberté ($df$)", "Carré moyen ($CM$)", "ratio $F$")
...
On peut réaliser ce genre d'exercice avec des boucles dans R. La section [Les boucles avec R](#BouclesR) couvre les concepts de base et donne des exemples pour construire des boucles qui permettront de réaliser des tâches répétitives.
...
out[i, 1] <- ifelse(comp1$p.value<=0.05, "<span>*</span>", "NS")
out[i, 2] <- ifelse(comp2$p.value<=0.05, "<span>*</span>", "NS")
out[i, 3] <- ifelse(comp3$p.value<=0.05, "<span>*</span>", "NS")
...
Error_comp1 <- sum(ifelse(out[,1]=="<span>*</span>", 1, 0))/nsims
##comparison-wise error rate of comparison 2
Error_comp2 <- sum(ifelse(out[,2]=="<span>*</span>", 1, 0))/nsims
##comparison-wise error rate of comparison 3
Error_comp3 <- sum(ifelse(out[,3]=="<span>*</span>", 1, 0))/nsims
...
A & \quad C & \quad B & \quad E & \quad D \\
3.1 & \quad 9.3 & \quad 10.5 & \quad 10.9 & \quad 11.8
\end{array}
\]
...
Les données peuvent être retrouvées dans le dossier [Module 6](https://github.com/sci1018/sci1018_data/raw/main/Module_6.zip).
