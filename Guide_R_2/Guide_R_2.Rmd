...
##### Vous utiliserez les fonctions suivantes : {-} 
  ...
  - `colSums( )`, `colMeans( )`
  - `data.frame( )`
  - `str( )`, `dim( )`, `nrow( )`, `ncol( )`, `names( )`, `rownames( )`, `colnames( )`
  - `summary( )`, `head( )`, `tail( )`
  - `read.table( )`
  - `setwd( )`
  - `subset( )`
...
Le fichier `vers.txt` dans la partie théorique qui contient des données sur l'abondance des vers de terre dans différents types de parcs publics et le fichier `richesse.txt` dans la partie exercices. 
Les données peuvent être téléchargées [ici](https://github.com/sci1018/sci1018_data/raw/main/Guide_R_2.zip).
  </div>
  
</div> 

## Notions générales {.unnumbered #NotionsR}
...
knitr::include_graphics("Guide_R_2/images/Capture_help1.png")
```
...
```{r exA1.62}
vers <- read.table(file = "Guide_R_2/data/vers.txt", 
                    header = TRUE)
``` 
...
tableau
save(tableau, file = "Guide_R_2/un.tableau.rdata")
``` 
...
Les données peuvent être retrouvées dans le dossier [Programmation avec R](https://github.com/sci1018/sci1018_data/raw/main/Guide_R_2.zip).  
