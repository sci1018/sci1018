# Tests d'hypothèse sur deux groupes {#lecon4}
...
<div class="row">
  ...
  ![](images/Icones_Bibliotheques.svg)
  </div>
 
  <div class="column righttext">
##### Vous utiliserez les packages `R` suivants : {-} 
  - `nortest`
  </div>
...
Le fichier `eleves.txt` est utilise dans la partie théorique et contient des données sur les notes des élèves d’une école primaire avec et sans aides aux devoirs pendant l’année. Le fichier `poissons.txt` est également utilisés et concerne l’effet des eaux usées sur le nombre d’espèces de poissons dans différents cours d’eau.   

Le fichier `recup.txt` est utilisé dans la partie Laboratoire. Il contient des données sur l’efficacité d’un médicament qui améliore le temps de récupération après un effort physique.   

Les données peuvent être téléchargées [ici](https://github.com/sci1018/sci1018_data/raw/main/Module_4.zip).
  </div>
...
$$
$$
s^2_{\mathrm{combin\acute{e}e}} = \frac{(30 - 1) \cdot `r round(var(df.aides), 2)` + (30 - 1) \cdot `r round(var(df.aides), 2)`}{30 + 30 - 2}
$$

$$
s^2_{\mathrm{combin\acute{e}e}} = `r round((29*var(df.aides) + 29*var(df.aides))/(30 + 30 - 2), 2)`
$$
...
SE_{\bar{x}_{\mathrm{aide}} -  \bar{x}_{\mathrm{t\acute{e}moins}}} = `r round(SE, 2)`
$$
...
t = \frac{`r round(mean(df.aides), 2)` - `r round(mean(df.temoins), 2)`}{`r round(SE, 2)`}\\
t = `r round(t.out$statistic, 3)`
$$
...
```{r normality, fig.align='center', fig.cap="Graphique quantile-quantile à partir des résidus du test $t$ sur deux groupes indépendants effectué sur les données du succès scolaire des élèves."}
...
```{r homog2, fig.align='center', fig.cap="Diagramme de boîtes et moustaches à partir des résidus du test $t$ sur deux groupes indépendants effectué sur les données du succès scolaire des élèves."}
...
```{r two, echo = TRUE, eval = TRUE, keep.source = TRUE, fig = TRUE, fig.align='center', fig.cap="Moyennes du temps passé sur leurs téléphones par des étudiants dînant accompagnés de compères et d'inconnus. Les barres d'erreurs correspondent à des intervalles de confiance à 95%."}
##moyenne des groupes
...
$$t = \frac{`r round(mean.diff, 1)` - 0}{`r round(SE, 2)` } $$
$$t =  `r round(t.poissons$statistic, 3)`$$
...
```{r norm6, echo = TRUE, keep.source = TRUE, eval = TRUE, fig = TRUE, fig.align='center', fig.cap="Vérification de la normalité à partir des résidus des données appariées sur le nombre d'espèces de poissons."}
##calcul des résidus
...
```{r pairedci, fig = TRUE, fig = TRUE, fig.align='center', fig.cap="Moyenne de la différence du nombre d'espèces de poissons (aval - amont) et intervalle de confiance à 95%."}
...
```{r hetlog, eval = TRUE, fig = TRUE, keep.source = FALSE, echo = FALSE, fig.align='center', fig.cap="Utilisation de la transformation logarithmique pour homogénéiser les variances de deux groupes. Les données brutes présentent des variances hétérogènes (a) puisque le groupe 1 varie moins que le groupe 2. L'application du log naturel a homogénéisé les variances (b), puisque la hauteur des deux boîtes est très semblable."}
set.seed(seed = 122)
...
boxplot(Val ~ Group, data = gg, 
        main = "Groupes avec variances hétérogènes", cex.main = 1)
text(x = 0.5, y = 295, labels = "a")
boxplot(log.Val ~ Group, data = gg, 
        main = "Groupes avec variances homogènes", cex.main = 1)
...
```{r sqrt, echo = TRUE, keep.source = TRUE, eval = TRUE, fig = TRUE, fig.align = 'center', fig.cap = "Distribution de Poisson avec une moyenne de 2.5. Fréquences originales (gauche) et fréquences transformées suivant la transformation racine carrée (droite)."}
...
```{r asin, echo = TRUE, keep.source = TRUE, eval = TRUE, fig = TRUE, fig.align = 'center', fig.cap = "Distribution bêta. Proportions originales (gauche) et proportions transformées suivant la transformation arcsinus (droite)."}
