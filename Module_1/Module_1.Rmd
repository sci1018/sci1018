...
# Statistiques descriptives {#lecon1}
...
Nous allons définir dans cette leçon certains termes et concepts en statistique.
...
\bar{x} = `r round(mean(c(12.3, 4.2, 5.9, 9.1, 3.3, 5.1, 7.3, 3.8, 8.0, 6.1)), digits = 2)`
$$
...
```{r histomode, echo=FALSE, fig.align='center', fig.cap="Histogramme illustrant une distribution unimodale (a) et bimodale (b)."}
##mode
...
```{r variance, echo=FALSE, fig.align='center', fig.cap="Effet du nombre d'observations sur l'estimation de la variance. À noter que la ligne pointillée représente la vraie valeur de la population ($\\sigma^2 = 4$) à partir de laquelle les observations ont été sélectionnées aléatoirement"}
...
```{r randomvar, echo=FALSE, fig.align='center', fig.cap="Présentation des longueurs de 100 serpents (variable continue, a) et du nombre de pucerons dans 100 sites (variable discrète, b)"}
...
$$
f(x \vert \mu, \sigma) = \frac{1}{\sigma \sqrt{2\pi}}e^{-\frac{1}{2}(\frac{x - \mu}{\sigma})^2} 
= \frac{\mathrm{exp}(-\frac{1}{2}(\frac{x - \mu}{\sigma})^2)}{\sigma \sqrt{2\pi}} 
$$
...
```{r normdensity, echo=FALSE, fig.align='center', fig.cap="Distribution normale d'une population de masses de souris où la moyenne est de 4.1 g et l'écart-type est de 1.5 g. À noter qu'on peut tracer la courbe de la distribution en substituant une série de valeurs de $x$ dans la fonction de densité de probabilité pour un $\\mu = 4.1$ et $\\sigma = 1.5$, c.-à-d., N(4.1, 1.5)."}
...
```{r shape, echo=FALSE, fig.align='center', fig.cap="Position de distributions normales avec même variance, mais différentes moyennes (a) et forme de distributions normales pour des variances différentes, mais une même moyenne (b)."}
...
```{r standardnormal, echo=FALSE, fig.align='center', fig.cap="Distribution normale centrée réduite, c.-à-d., N(0, 1)."}
##distribution normale centrée-réduite
x <- seq(-3, 3, 0.1)
...
```{r zex1, echo=FALSE, fig.align='center', fig.cap="Écart normal associé à une longueur d'ailes de puceron de 22.6 mm."}
...
```{r zex2, echo=FALSE, fig.align='center', fig.cap="Probabilité cumulative associée à une valeur $\\leq$ 160 cm dans une population avec N(170, 8)."}
...
Ici, $P$ est une probabilité cumulative que l'on peut obtenir en calculant l'aire sous la courbe pour la portion de la courbe à gauche du point -1.25 (Figure \@ref(fig:zex2)).  
:::
...
Toutefois, nous désirons $P(x_i > 185 \: \mathrm{cm})$, ce qui diffère des exemples précédents avec $P(x_i \leq X)$ (Figure \@ref(fig:zex3)a). L'astuce ici consiste à calculer le complément de $P(x_i \leq 185 \: \mathrm{cm})$ (Figure \@ref(fig:zex3)b). Puisque l'aire sous la courbe est de 1, on peut obtenir $P(x_i > 185 \: \mathrm{cm})$ simplement avec:

```{r zex3, echo=FALSE, fig.align='center', fig.cap="Probabilité cumulative associée à une valeur > 185 cm dans une population avec N(170, 8)."}
par(mfrow = c(1, 2), cex = 1.2)
##exemple 2
z <- (185 - 170)/8
...
```{r zex4, echo=FALSE, fig.align='center', fig.cap="Probabilité cumulative associée à 165 cm $\\leq$ $\\ x_i$ $\\leq$ 180 cm dans une population avec N(170, 8)."}
...
:::{.exemple section=1}
Nous voulons déterminer la probabilité d'obtenir un échantillon aléatoire de 9 longueurs de becs d'oiseaux, lequel a une moyenne > 50.0 mm dans une population avec $\mu = 47.5 \: \mathrm{mm}$ et $\sigma = 12.89 \: \mathrm{mm}$. On obtient:
