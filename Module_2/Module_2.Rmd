...
Le jeu de données `croissance.csv` sur la croissance en kg de bovins auxquels on a donné l'une des trois types de moulées. 
Les données peuvent être téléchargées [ici](https://github.com/sci1018/sci1018_data/raw/main/Module_2.zip).
  </div>
...
```{r uniform, echo=FALSE, fig.align='center', fig.cap="Distribution uniforme pour l'intervalle 0 et 10."}
...
```{r t, echo=FALSE, fig.align = 'center', fig.cap="Comparaison de la distribution normale centrée réduite et de la distribution du $t$ de Student. On note qu'il y a un plus grand nombre de valeurs dans les extrémités de la distribution du $t$ que dans la distribution normale centrée réduite dès que $n < 50$."}
xvs <- seq(-4, 4, 0.01)
...
```{r qt, echo=FALSE, fig.align = 'center', fig.cap="Valeurs des quantiles de la distribution du $t$ de Student en fonction de la taille d'échantillon ($df = n - 1$) pour une probabilité cumulative de 0.95. La ligne horizontale pointillée correspond à l'écart-normal de la distribution normale centrée réduite pour la même probabilité cumulative."}
...
```{r semis, echo=FALSE, fig.align = 'center', fig.cap="Distribution de la population complète de semis dans une plantation où $\\mu = 74.99$ et $\\sigma = 14.99$ (a) et d'un échantillon aléatoire de 25 observations tiré de cette population (b)."}
...
moy1 <- mean(echantillon.1)
ecartype1 <- sd(echantillon.1)
...
$$
\bar{x} = `r round(moy.1, 2)` \: \text{cm} \\
s = `r round(ecartype.1, 2)` \: \text{cm} \\
n = 25 \\
SE = \frac{`r round(ecartype.1, 2)`}{\sqrt{25}} =  `r round(SE, 2)` \: \text{cm} \\
  \\ 
IC \: \text{à} \: 95\:\%: \\
P(\bar{x} - t_{0.05/2, (25 - 1)} \cdot SE \leq \mu \leq \bar{x} + t_{0.05/2, (25 - 1)} \cdot SE) = 0.95\\
P(`r round(moy.1, 2)` - `r round(qt(p = 0.025, df = 24, lower.tail = FALSE), 2)` \cdot `r round(SE, 2)`) \leq \mu \leq `r round(moy.1, 2)` + `r round(qt(p = 0.025, df = 24, lower.tail = FALSE), 2)` \cdot `r round(SE, 2)`) = 0.95\\
IC \: \text{à} \: 95\%: (`r round(lci, 2)`, `r round(uci, 2)`)
$$
...
```{r SE, echo=FALSE, fig.align = 'center', fig.cap="Distribution des 1000 moyennes d'échantillons de 25 semis tirés de la population de 200 000 semis."}
...
```{r cluster, fig.align='center', echo=FALSE, fig.link='images/Target3.png', fig.cap="Exemple de grappes de quadrats pour l'échantillonnage par grappes."}
...
```{r systematic, fig.align='center', echo=FALSE, fig.link='images/Target3.png', fig.cap="Exemple d'échantillonnage systématique avec deux grappes (grilles) de quadrats."}
...
```{r hist, echo=TRUE, fig.align='center', fig.cap="Histogramme des valeurs de superficie dans le jeu de données de vers de terre."}
vers <- read.table(file = "Module_2/data/vers.txt", header = TRUE) 
hist(vers$Superficie) 
```
```{r nicehist, echo=TRUE, fig.align='center', fig.cap="Histogramme plus élaboré à partir des valeurs de superficie dans le jeu de données de vers de terre."}
...
```{r barplot, echo=TRUE, fig.align='center', fig.cap="Diagramme à bâtons des fréquences de différentes classes de végétation dans le jeu de données de vers de terre."}
...
```
```{r barplotb, echo=TRUE, fig.align='center', fig.cap="Diagramme à bâtons avec différentes structures et couleurs à partir des fréquences de chaque classe de végétation."}
...
```{r boxplot, echo=TRUE, fig.align='center', fig.cap="Diagramme de boîtes et moustaches de densités de vers de terre pour chaque classe de végétation."}
...
```{r plot, echo=TRUE, fig.align='center', fig.cap="Différents types de graphiques obtenus avec la fonction `plot( )`."}
...
Les données peuvent être retrouvées dans le dossier [Module 2](https://github.com/sci1018/sci1018_data/raw/main/Module_2.zip).  
