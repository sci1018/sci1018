# Élaboration et optimisation de plans d'échantillonnage
...
18. La fréquence d’un mot ou d’un groupe de mots dans différents journaux selon le sujet abordé ou depuis l’avènement d’un événement important (p. ex., nomination d’un nouveau premier ministre)^[La fréquence d’un mot ou d’un groupe de mots peut être évaluée grâce à des logiciels d'analyse de texte, tels que `TextSTAT` ((http://neon.niederlandistik.fu-berlin.de/en/textstat/)) et `Textalyser` [http://textalyser.net/](http://textalyser.net/).].
