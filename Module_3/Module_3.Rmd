# Tests d'hypothèse sur un seul groupe {#lecon3}
...
Le fichier `vers.txt` dans la partie théorique qui contient des données sur l'abondance des vers de terre dans différents types de parcs publics et le fichier `temp.csv` dans la partie Autoévaluation qui contient des données sur la température de l'air pour différentes stations de mesures météorologiques dans l'Arctique.  

Les données peuvent être téléchargées [ici](https://github.com/sci1018/sci1018_data/raw/main/Module_3.zip).
  </div>
...
df <- data.frame(
  ...
  Col3 = c("On ne rejette pas $H_0$", 
           cell_spec("décision correcte", color = "blue"),
           cell_spec("erreur de type II ($\\beta$) \n (faux négatif)", color = "red")),
  Col4 = c("On rejette $H_0$", 
           cell_spec("erreur de type I ($\\alpha$) \n (faux positif)", color = "red"), 
           cell_spec("décision correcte \n (puissance = 1 - $\\beta$)", color = "blue"))
  )
...
```{r qqplot, echo = FALSE, fig.align='center', fig.cap="Graphique quantile-quantile pour évaluer la normalité des résidus du test-$t$ sur les données de perte de masse (a) et un échantillon qui ne suit pas la normalité (b)."}
par(mfrow = c(1, 2))
##un graphique quantile-quantile
qqnorm(residus, main = "Graphique quantile-quantile",
       ylab = "Quantiles de l'échantillon",
...
```{r qqplot2, echo=TRUE, fig.align='center', fig.cap="Graphique quantile-quantile pour évaluer la normalité des résidus du test-$t$ de l'âge d'ours de Montérégie."}
##on crée un vecteur avec les observations
age.ours <- c(18, 13, 17, 2, 8, 14, 8, 5, 9, 2, 8, 7, 7, 
              3, 19, 21, 20, 11, 7, 16)
...
Les données peuvent être retrouvées dans le dossier [Module 3](https://github.com/sci1018/sci1018_data/raw/main/Module_3.zip).    
